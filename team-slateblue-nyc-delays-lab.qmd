...
# Import carriers lookup table
carriers <- read_csv("carriers.csv")

# Build departures tibble
departures <- flights |>
  # Drop cancelled flights (NA in dep_delay)
  filter(!is.na(dep_delay)) |>
  # Select and rename/create required columns
  transmute(
...
# Checking dataset before Joining with csv file (after filter + transmute)
pre_join <- flights |>
  filter(!is.na(dep_delay)) |>
  transmute(
    airline_code = carrier,
...
rows_before_join <- nrow(pre_join)
...
# Print summary of checks if failed
stopifnot(
  "Row count changed after join" = rows_before_join == rows_after_join,
  "Duplicate codes in carriers lookup" = nrow(duplicate_codes) == 0,
  "Missing values present in departures" = all(na_counts == 0)
)
...
Airline names are long character strings that overlap heavily with one another when placed on the x-axis, forcing diagonal or rotated tick labels that are harder to read. Placing them on the y-axis allows each label to sit on its own horizontal
line, left-aligned and fully legible without rotation.
...
A lollipop chart does not require a zero baseline as strongly as a bar chart
does, which can encourage designers to omit it. However, unlike a bar chart with it has filled area and a truncated axis is immediately misleading. A lollipop's stem still must start at zero, otherwise the length of the stem no longer encodes the count accurately. Readers are less familiar with lollipop conventions may not notice a missing or shifted baseline.
...
Cleveland dot plot encodes values through the position along a common scale, not through length from a baseline. Readers compare dot positions relative to one another, not relative to the origin like other charts, so a non-zero starting point does not distort the encoded quantities. The axis can legitimately begin
near the smallest data value to spread the points apart and make small
differences easier to perceive.
...
**e. Based on the stacked bar chart, which airport does the busiest airline depart from most, and how lopsided is that share?**

```{r}
#| label: busiest-dominant-airport
busiest_airport_split <- airport_mix |>
  filter(airline == busiest_airline) |>
  arrange(desc(pct)) |>
  select(airline, airport, n, pct)

print(busiest_airport_split)
```

Busiest Airline as found earilier: United Airlines.
Based on the stacked bar chart and the summary table above, the busiest airline ,United Airlines, departs mostly from **Newark (EWR)** — its dominant airport — with approximately 80% or more of its departures originating there. The remaining share is split very thinly across JFK and LaGuardia, making United's exposure heavily concentrated at a single airport. 
...
### b. Dominant Airport vs. On-Time Performance
```{r}
#| label: dominant-airport-check
# Identify the busiest airline's dominant airport
dominant_airport <- airport_mix |>
  filter(airline == busiest_airline) |>
  arrange(desc(pct)) |>
  slice(1) |>
  pull(airport)

# On-time percentage of the dominant airport
dominant_airport_pct <- ontime_by_airport |>
  filter(airport == dominant_airport) |>
  pull(on_time_pct)

# Rank of the dominant airport by on-time percentage (1 = highest)
airport_rank <- ontime_by_airport |>
  arrange(desc(on_time_pct)) |>
  mutate(rank = row_number()) |>
  filter(airport == dominant_airport) |>
  select(airport, on_time_pct, rank)

cat("Busiest airline:          ", busiest_airline, "\n")
cat("Dominant airport:         ", dominant_airport, "\n")
cat("Dominant airport on-time %:", round(dominant_airport_pct, 2), "\n\n")

cat("Airport on-time ranking:\n")
print(
  ontime_by_airport |> arrange(desc(on_time_pct)) |> mutate(rank = row_number())
)
```

Dominate Airport of busiest airline: Newark (EWR)
On-time percentage of Newark: 74.7% (lowest among the three airports)

- If the dominant airport is also the lowest punctuality airport, the busiest airline faces a structural disadvantage in the raw ranking that has nothing to do with its own scheduling decisions. Because the vast majority of its departures originate from the worst performing airport, its pooled on-time rate is dragged downward by the airport's baseline performance rather than by any failure on the airline's part which is not influenced by the airline's scheduling decisions!!

- This suggests the raw rank is at least partly confounded by airport mix where an airline flying 80% or more of its routes from a low-punctuality hub will naturally post a lower raw on-time rate than an otherwise equally punctual airline whose departures are concentrated at a more punctual airport.

- **A fair comparison requires controlling for airport exposure:** To assess
  whether the busiest airline is genuinely less punctual than its
  competitors — or merely penalised by its dominant airport — its observed
  on-time rate must be compared against the rate that would be expected
  given the airports and hours it flies. This adjustment is the subject of
  the remaining tasks.
